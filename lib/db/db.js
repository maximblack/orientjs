...
inherits(LiveQuery, EventEmitter);

LiveQuery.prototype.handleLiveQuery = function (currentToken, operation, result) {

  if (currentToken == this.config.token) {
    if (operation === 1) {
      this.emit("live-update", result, currentToken);
    } else if (operation === 2) {
      this.emit("live-delete", result, currentToken);
    } else if (operation === 3) {
      this.emit("live-insert", result, currentToken);
      this.emit("live-create", result, currentToken);
    }
  }
};
 ...
 * @param   {String} query   The query or command to execute.
 * @param   {Object} options The options for the query / command.
 * @return {LiveQuery}          The token of the live query.
 */
Db.prototype.liveQuery = function (command, options) {
  options = options || {};
  options.mode = 'l';
  options.class = 'q';
  var live = new LiveQuery();
  this.exec(command, options)
    ...
    .then(function (response) {
      if (response.length > 0) {
        ...
        this.server.transport.connection.on("live-query-result", function (token, operation, result) {
          live.handleLiveQuery(token, operation, result);
        });
